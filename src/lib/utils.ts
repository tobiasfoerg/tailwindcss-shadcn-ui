...
import { Colors, HslString, Theme, ThemeConfig } from "./types";
...
function colorsToCSSRuleObject(colors: Colors) {
	return Object.entries(colors).reduce<{ [P in keyof Colors]: HslString }>((agg, [key, value]) => {
		agg[key as keyof Colors] = typeof value === "string" ? value : `${value[0]} ${value[1]}% ${value[2]}%`;
		return agg;
	}, {} as any);
}
